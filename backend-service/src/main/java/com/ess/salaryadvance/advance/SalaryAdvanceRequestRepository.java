...
public interface SalaryAdvanceRequestRepository extends JpaRepository<SalaryAdvanceRequest, Long> {
    ...
    List<SalaryAdvanceRequest> findByStatusOrderByUpdatedAtAsc(RequestStatus status);

    List<SalaryAdvanceRequest> findByCreatedAtBetweenOrderByCreatedAtDesc(Instant start, Instant end);

    List<SalaryAdvanceRequest> findByStatusAndCreatedAtBetweenOrderByCreatedAtDesc(RequestStatus status, Instant start,
            Instant end);
}
